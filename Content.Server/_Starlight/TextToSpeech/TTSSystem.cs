...
{  
    ...
    private const string DefaultAnnounceVoice = "scout";  
    ...
    {  
        ...
        {  
            HandleRadio(args.Receivers, args.Message, DefaultAnnounceVoice, chime, args.Language);  
        }  
        else  
        {  
            HandleRadio(args.Receivers, args.Message, voiceId, chime, args.Language);  
        }  
    }  
    ...
    {  
        ...
        if (!TryComp(args.Source, out TextToSpeechComponent? senderComponent)  
            || senderComponent.VoicePrototypeId is not string voiceId)  
        {  
            HandleCollectiveMind(args.Receivers, args.Message, DefaultAnnounceVoice);  
        }  
        else  
        {  
            HandleCollectiveMind(args.Receivers, args.Message, voiceId);  
        }  
    }  
    ...
    {  
        ...
        var voiceId = args.AnnounceVoice ?? DefaultAnnounceVoice;  
  
        var soundData = await GenerateTTS(args.Message, voiceId, isAnnounce: true);  
        soundData ??= [];  
    ...
    {  
        ...
        {  
            ...
            {  
                ...
                {  
                    ...
                    voiceId = prototype.Value.ID;  
                    component.VoicePrototypeId = prototype.Value.ID;  
                }  
                else  
                {  
                    voiceId = DefaultAnnounceVoice;  
                }  
            }  
            else  
            {  
        ...
        if (args.IsWhisper)  
        {  
            HandleWhisper(uid, args.Message, voiceId, args.Language);  
            return;  
        }  
  
        HandleSay(uid, args.Message, voiceId, args.Language);  
    }  
    ...
    private async void HandleSay(EntityUid uid, string message, string voiceId, LanguagePrototype language)  
    {  
    ...
    private async void HandleWhisper(EntityUid uid, string message, string voiceId, LanguagePrototype language)  
    {  
        var soundData = await GenerateTTS(message, voiceId);  
        if (soundData is null)  
            return;  
    ...
    private async void HandleRadio(EntityUid[] uIds, string message, string voiceId, SoundSpecifier? chime, LanguagePrototype language)  
    {  
        ...
        var soundData = await GenerateTTS(message, voiceId, isRadio: true);  
        if (soundData is null)  
            return;  
    ...
    private async void HandleCollectiveMind(EntityUid[] uIds, string message, string voiceId)  
    {  
        var soundData = await GenerateTTS(message, voiceId, isRadio: true);  
        if (soundData is null)  
            return;  
    ...
    private async Task<byte[]?> GenerateTTS(string text, string voiceId, bool isRadio = false, bool isAnnounce = false)  
    {  
        try  
        {  
            ...
            return isRadio  
                ? await _ttsManager.ConvertTextToSpeechRadio(voiceId, text)  
                : isAnnounce  
                    ? await _ttsManager.ConvertTextToSpeechAnnounce(voiceId, text)  
                    : await _ttsManager.ConvertTextToSpeechStandard(voiceId, text);  
        }  
        catch (Exception e)  
        {  
    ...
    [GeneratedRegex(@"[^a-zA-Zа-яёА-ЯЁ0-9,\-+?!. ]")]  
