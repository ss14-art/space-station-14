...
def find_top_level_dir(start_dir):
    ...
    while True:
        ...
        if parent_dir == current_dir:
            print(f"Не удалось найти корень проекта начиная с {start_dir}")
            exit(-1)
...
def remove_duplicates(root_dir):
    ...
    log_filename = os.path.join(logs_dir, f"removed_duplicates_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
