import os
import yaml  # Добавил импорт для отлова ошибок
    ...
    def execute(self):
        for yaml_file in self.yaml_files:
            try:
                ...
                raw_data = yaml_file.read_data()
                
                try:
                    # Пробуем распарсить как есть
                    parsed_yaml = yaml_file.parse_data(raw_data)
                except yaml.scanner.ScannerError as e:
                    # Если нашли табы (character '\t'), фиксим их на лету
                    if "found character '\\t'" in str(e) or "found character '\t'" in str(e):
                        logging.warning(f"Исправляю табы в файле: {yaml_file.full_path}")
                        fixed_data = raw_data.replace('\t', '    ')
                        parsed_yaml = yaml_file.parse_data(fixed_data)
                    else:
                        raise e # Если ошибка другая, прокидываем выше

                yaml_elements = yaml_file.get_elements(parsed_yaml)
                ...
                # Если файл совсем битый, не падаем, а идем дальше
                logging.error(f"Ошибка при обработке файла {yaml_file.full_path}: {e}")
                continue
